...
class Enregistrement:
    def __init__(self,root):
        ...
        title=Label(self.root,text="Liste de divers activités de Tadjouah",bd=10,relief=GROOVE,font=("times new roman",30,"bold"),bg="yellow",fg="red")
    ...
    def ajouter(self):
        if self.No_var.get()=="" or self.nom_var.get()=="":
            ...
            cur.execute("insert into DiversTadjouah values(?,?,?,?,?,?,?,?)",(n,nom,ad,ac,genre,cni,contact,montant))                                           
            con.commit()
            self.fetch_data()
            self.clear()
            con.close()
    ...
    def fetch_data(self):
        con=sqlite3.connect('New_work.db')
        cur=con.cursor()
        cur.execute("select * from DiversTadjouah")
        rows=cur.fetchall()
        if len(rows)!=0:
    ...
    def modifier_data(self):
        ...
        contact=self.contact_var.get()
        montant=self.montant_var.get()
        cur.execute("update DiversTadjouah set Nom=?,Adresse=?,Activité=?,Genre=?,N°CNI=?,N°Téléphone=?,Montant=? where No=?",(nom,ad,ac,genre,cni,contact,montant,n))                                           
        con.commit()
        self.fetch_data()
        self.clear()
        con.close()
    ...
    def supprimer(self):
        con=sqlite3.connect('New_work.db')
        cur=con.cursor()
        cur.execute("delete from DiversTadjouah where No=?",self.No_var.get())
        con.commit()
        self.fetch_data()
        self.clear()
        con.close()
    ...
    def search(self):
        try: 
            con=sqlite3.connect('New_work.db')
            cur=con.cursor()
            sql="select * from DiversTadjouah where "+str(self.search_by.get())+" LIKE '%"+str(self.search_txt.get())+"%'"
            cur.execute(sql)
            rows=cur.fetchall()
            if len(rows)!=0:
                    self.manage_table.delete(*self.manage_table.get_children())
