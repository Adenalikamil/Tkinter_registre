...
class Enregistrement:
    def __init__(self,root):
        ...
        title=Label(self.root,text="Association des Femmes de Tadjourah",bd=10,relief=GROOVE,font=("times new roman",30,"bold"),bg="yellow",fg="red")
        ...
        m_title=Label(Manage_frame,text="Manage personne",bg="crimson",fg="white",font=("time new roman",20,"bold"))
        ...
        lb1_recherche=Label(Detail_frame,text="Recherche",bg="crimson",fg="white",font=("time new roman",10,"bold"))
        lb1_recherche.grid(row=0,column=0,pady=10,padx=20,sticky="w")

        combo_recherche=ttk.Combobox(Detail_frame,textvariable=self.search_by,width=10,font=("time new roman",10,"bold"),state='readonly')
        ...
        recherchebtn=Button(Detail_frame,text="Recherche",width=6,pady=5,command=self.search).grid(row=0,column=3,padx=10,pady=10)
    ...
    def ajouter(self):
        if self.No_var.get()=="" or self.nom_var.get()=="":
            ...
            cur.execute("insert into Tadjourah values(?,?,?,?,?,?,?,?)",(n,nom,ad,ac,genre,cni,contact,montant))                                           
            con.commit()
            self.fetch_data()
            self.clear()
            con.close()
    ...
    def fetch_data(self):
        con=sqlite3.connect('New_work.db')
        cur=con.cursor()
        cur.execute("select * from Tadjourah")
        rows=cur.fetchall()
        if len(rows)!=0:
    ...
    def modifier_data(self):
        ...
        contact=self.contact_var.get()
        montant=self.montant_var.get()
        cur.execute("update Tadjourah set Nom=?,Adresse=?,Activité=?,Genre=?,N°CNI=?,N°Téléphone=?,Montant=? where No=?",(nom,ad,ac,genre,cni,contact,montant,n))                                           
        con.commit()
        self.fetch_data()
        self.clear()
        con.close()
    ...
    def supprimer(self):
        con=sqlite3.connect('New_work.db')
        cur=con.cursor()
        cur.execute("delete from Tadjourah where No=?",self.No_var.get())
        con.commit()
        self.fetch_data()
        self.clear()
        con.close()
    ...
    def search(self):
        try: 
            con=sqlite3.connect('New_work.db')
            cur=con.cursor()
            sql="select * from Tadjourah where "+str(self.search_by.get())+" LIKE '%"+str(self.search_txt.get())+"%'"
            cur.execute(sql)
            rows=cur.fetchall()
            if len(rows)!=0:
                    self.manage_table.delete(*self.manage_table.get_children())
